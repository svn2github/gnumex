...
if (pps.mingwf == 1)
  fp('rem Mingw compile / link');
elseif (pps.mingwf == 2)
  fp('rem Cygwin (cygwin*.dll) linking');
else
  fp('rem Cygwin / mingw (-mno-cygwin) linking');
end
if (pps.mexf == 1)
  fp('rem Mex file creation');
else
  fp('rem Engine / mat file creation');
end
if (pps.safef == 1)
  fp('rem Standard (safe) linking to temporary libraries');
else
  fp('rem Non standard (fast) linking to precompiled libraries');
end
if (pps.lang == 1)
  fp('rem Language C / C++');
else
  fp('rem Language Fortran');
end
...
if ~isempty(fn),
  ...
  [ok_opts opt_inds] = ismember(varargin, cmd_opts(:,1));
  if ~all(ok_opts)
